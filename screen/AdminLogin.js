...
const AdminLoginScreen = ({ navigation }) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      const response = await fetch('http://10.54.12.248:3001/admin-login', {
        method: 'POST',
      ...
      if (data.success) {
        ...
        navigation.navigate('AdminDashboard'); // Replace with your admin dashboard screen
      } else {
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity onPress={() => navigation.navigate('AdminPasswordReset')}>
